...
mod_v3 <- mgcv::gamm(value ~ s(doy, by = rot_trt, bs = "cr", k = 35) + rot_trt,
                     random = list(block = ~ 1),
                     data = sw_d15y18, method = "REML")
nlraa::IC_tab(mod_v2, mod_v3, criteria = "BIC")
...
## Looking at the data by plot_id
ggplot(data = sw_d15y19, aes(x = doy, y = value, color = plot_id)) + 
  geom_point()

## Looking at the data by trt
ggplot(data = sw_d15y19, aes(x = doy, y = value, color = rot_trt)) + 
  geom_point()
...
## The default number of knots was 70, that is too high
mod_v19 <- mgcv::gam(value ~ s(doy, by = rot_trt, bs = "cr", k = 35) + rot_trt,
                   data = sw_d15y19, method = "REML")

mod_v19_prd <- predict(mod_v19, se.fit = TRUE)
sw_d15y19A <- cbind(sw_d15y19, 
                    Estimate = mod_v19_prd$fit, 
                    Q2.5 = mod_v19_prd$fit - 1.96 * mod_v19_prd$se.fit,
                    Q97.5 = mod_v19_prd$fit + 1.96 * mod_v19_prd$se.fit)

## GAM fit with 95% confidence bands
ggplot(data = sw_d15y19A, aes(x = doy, y = value, color = rot_trt)) + 
  geom_point(alpha = 0.2) + 
  geom_line(aes(y = Estimate)) + 
  geom_ribbon(aes(ymin = Q2.5, ymax = Q97.5, fill = rot_trt, color = NULL), alpha = 0.3)

## How does the model change when we incorporate the random effect of block?
mod_v19_2 <- mgcv::gam(value ~ s(doy, by = rot_trt, bs = "cr", k = 35) + rot_trt + s(block, bs = "re"),
                    data = sw_d15y19, method = "REML")

mod_v19_2_prd <- predict(mod_v19_2, se.fit = TRUE, exclude = "s(block)")
sw_d15y19A2 <- cbind(sw_d15y19, 
                     Estimate = mod_v19_2_prd$fit, 
                     Q2.5 = mod_v19_2_prd$fit - 1.96 * mod_v19_2_prd$se.fit,
                     Q97.5 = mod_v19_2_prd$fit + 1.96 * mod_v19_2_prd$se.fit)

ggplot(data = sw_d15y19A2, aes(x = doy, y = value, color = rot_trt)) + 
  geom_point(alpha = 0.2) + 
  geom_line(aes(y = Estimate)) + 
  geom_ribbon(aes(ymin = Q2.5, ymax = Q97.5, fill = rot_trt, color = NULL), alpha = 0.3)

## What does a GAMM model look like?
mod_v19_3 <- mgcv::gamm(value ~ s(doy, by = rot_trt, bs = "cr", k = 35) + rot_trt,
                     random = list(block = ~ 1),
                     data = sw_d15y19, method = "REML")

mod_v19_3_prd <- predict(mod_v19_3$gam, se.fit = TRUE)
sw_d15y19A3 <- cbind(sw_d15y19, 
                     Estimate = mod_v19_3_prd$fit, 
                     Q2.5 = mod_v19_3_prd$fit - 1.96 * mod_v19_3_prd$se.fit,
                     Q97.5 = mod_v19_3_prd$fit + 1.96 * mod_v19_3_prd$se.fit)
...
sw_d15y18_res %>% 
  bind_rows(sw_d15y19_res) %>% 
  write_csv("01_soilsens/dat_soilsens-gam-fem.csv")
