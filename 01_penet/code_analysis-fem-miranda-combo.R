...
# Fit and check GAM
# The general format of a gam() call is gam(y ~ s(x1, by = x2, k = 15) + x2, data = data), where x terms within s() control the shape of the wiggly curve(s), x terms outside of s() control the intercept(s) (if desired), and k controls the number of knots. Also, I think you need x1 to be continuous and x2 to be a factor/category, but I admittedly had trouble figuring this out...

library(mgcv)
# this takes a long time to fit, and we wind up getting rid of 2020 anyways
