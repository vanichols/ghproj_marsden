...
# Soil moisture
#
sm <- read_xlsx("../2018_field-season/_data_raw/dat_mars-soil_penetrometer.xlsx", skip = 5, na = "NA") %>%
  mutate(samp_ID = paste(block, trt, N, sep = "-"),
         
         #Calculate %mois by weight
         soilmois_g.g = 1 - (drysoil_g - drybag_g) / (wetsoil_g - wetbag_g)) %>%
...
# Penetrometer
#
olk <- read_xlsx("../2018_field-season/_data_raw/dat_mars-soil_penetrometer-OLK.xlsx", skip = 5) %>%
...
ggsave("../_figs/fig_MARSline_penet-trt.png", width = 4.24, height = 7, units = "in")
...
ggsave("../_figs/fig_MARSline_penet-trt-mois.png", width = 4.24, height = 7, units = "in")
...
ggsave("../_figs/fig_MARSline_mois-penet.png", width = 4.24, height = 7, units = "in")
...
dat %>% mutate(depth_cm = depth_cm - 2.54) %>%
  ...
  labs(y = "Resistance [PSI]", x = "Depth [cm]", color = "Treatment") + 
  scale_color_manual(values = c("darkgoldenrod1", "cyan3"))

ggsave("../_figs/fig_MARSline_penet-trt-cm.png", width = 4.24, height = 7, units = "in")
...
# Penetrometer
#
poke <- read_xlsx("../2018_field-season/_data_raw/dat_mars-soil_penetrometer-SPECTRUM-07-16-18.xlsx", skip = 5) %>%
...
ggsave("../_figs/fig_MARS-penet-071618.png", width = 4.24, height = 7, units = "in")
